...
def position_marker(chain, MMoutput):
    ...
    for index, current in enumerate(chain):
        ...
        if MMoutput[index] in Black: # Black
            ...
            #print "It's Blue"
            if (index == len(chain) - 1) and (MMoutput[index-1] in Red):
                marked_chain[index][1] = 1
                continue
            elif (index == len(chain) -1) and not(MMoutput[index-1] in Red):
...
def codonize(marked_chain):
    ...
    for index, elem in enumerate(marked_chain):
        if index == len(marked_chain) - 1 and elem[1]==1:
            codonized_chain.append(codon)
            codon = []
            codon.append(elem)
            codonized_chain.append(codon)
        elif index == len(marked_chain) - 1:
            codon.append(elem)
            codonized_chain.append(codon)
        elif index == 0:
            codon.append(elem)
...
def process_del(codonized_chain, backspace):
    ...
    if len(last) == 1:
        ...
        elif (backspace > 1) and (backspace-1 < len(codonized_chain[-2])):
            processed_codonized_chain.pop()
            for i in range(backspace - 1):
                processed_codonized_chain[-1].pop()
        elif backspace >= len(codonized_chain[-2])+1:
            length = len(codonized_chain[-2])
            processed_codonized_chain.pop()
            processed_codonized_chain.pop()
            print processed_codonized_chain
            for i in range(backspace - 1 - length):
                processed_codonized_chain.pop()
    else:
